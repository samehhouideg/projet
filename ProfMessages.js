...
function ProfMessages() {
  ...
  // جميع الرسائل من قاعدة البيانات
  const allMessages = DB.messages || [];

  // تصفية الرسائل بين الأستاذ وجهة الاتصال المحددة
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: '#f8fafc' }}>
      <ProfSidebar activeTab="messages" />
      
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '32px 40px', overflow: 'hidden' }}>
        
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '28px', fontWeight: 'bold', color: '#0f172a' }}>Messages</h1>
          <p style={{ fontSize: '15px', color: '#64748b' }}>Communiquez avec vos étudiants et collègues.</p>
        </div>
              ...
              {contacts.map(contact => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: '#64748b' }}>{contact.role === 'etudiant' ? '🎓 Étudiant' : contact.role === 'admin' ? '👤 Admin' : '👨‍🏫 Professeur'}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {selectedContactId ? (
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8' }}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: '14px' }}>Choisissez un étudiant ou un collègue pour commencer à discuter.</p>
                </div>
              </div>
            )}
          </div>
